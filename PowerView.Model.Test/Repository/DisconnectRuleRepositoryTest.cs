...
namespace PowerView.Model.Test.Repository
{
    ...
    public class DisconnectRuleRepositoryTest : DbTestFixtureWithSchema
    {
        ...
        public void AddDisconnectRuleUpdates()
        {
            ...
            var dbDisconnectRule = new Db.DisconnectRule
            {
                Label = sn1.Label,
                ObisCode = sn1.ObisCode,
                EvaluationLabel = sn1.Label,
                EvaluationObisCode = sn1.ObisCode,
                DurationSeconds = 4,
                DisconnectToConnectValue = 4,
                ConnectToDisconnectValue = 3,
                Unit = 4
            };
        ...
        DurationSeconds=@TotalSeconds AND DisconnectToConnectValue=@DisconnectToConnectValue AND ConnectToDisconnectValue=@ConnectToDisconnectValue AND Unit=@Unit;",
              new
              {
                  disconnectRule.Name.Label,
                  ObisCode = (long)disconnectRule.Name.ObisCode,
                  EvaluationLabel = disconnectRule.EvaluationName.Label,
                  EvaluationObisCode = (long)disconnectRule.EvaluationName.ObisCode,
                  TotalSeconds = (int)disconnectRule.Duration.TotalSeconds,
                  disconnectRule.DisconnectToConnectValue,
                  disconnectRule.ConnectToDisconnectValue,
                  Unit = (byte)disconnectRule.Unit
              });
        ...
        {
            // Arrange
            var dbDr1 = new Db.DisconnectRule
            {
                Label = "lbl1",
                ObisCode = (ObisCode)"0.1.96.3.10.255",
                EvaluationLabel = "lbl11",
                EvaluationObisCode = 11,
                DurationSeconds = 1111,
                DisconnectToConnectValue = 11,
                ConnectToDisconnectValue = 1,
                Unit = 1
            };
            var dbDr2 = new Db.DisconnectRule
            {
                Label = "lbl1",
                ObisCode = 2,
                EvaluationLabel = "lbl22",
                EvaluationObisCode = 22,
                DurationSeconds = 2222,
                DisconnectToConnectValue = 22,
                ConnectToDisconnectValue = 2,
                Unit = 2
            };
            var dbDr3 = new Db.DisconnectRule
            {
                Label = "lbl2",
                ObisCode = (ObisCode)"0.1.96.3.10.255",
                EvaluationLabel = "lbl33",
                EvaluationObisCode = 33,
                DurationSeconds = 3333,
                DisconnectToConnectValue = 33,
                ConnectToDisconnectValue = 3,
                Unit = 3
            };
        ...
        public void GetLatestSerieNamesFromLive()
        {
            ...
            Assert.That(res, Contains.Key(new SeriesName("Label-1", ObisCode.ElectrActualPowerP23L2)));
