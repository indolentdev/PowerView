...
public class SettingRepositoryTest : DbTestFixtureWithSchema
{
    ...
    private void AssertEnergiDataServiceImportConfig(bool importEnabled, TimeSpan timeSpan, string priceArea, string label, Unit currency)
    {
        ...
        AssertExists("EDSI_ImportEnabled", importEnabled.ToString(CultureInfo.InvariantCulture));
        AssertExists("EDSI_TimeSpan", timeSpan.ToString());
        AssertExists("EDSI_PriceArea", priceArea);
        AssertExists("EDSI_Label", label);
        AssertExists("EDSI_Currency", currency.ToString());
    }
    ...
    public void GetEnergiDataServiceImportConfig()
    {
        ...
        Assert.That(edsiConfig.TimeSpan, Is.EqualTo(TimeSpan.FromHours(1)));
        Assert.That(edsiConfig.PriceArea, Is.EqualTo("DK2"));
        Assert.That(edsiConfig.Label, Is.EqualTo("Main"));
