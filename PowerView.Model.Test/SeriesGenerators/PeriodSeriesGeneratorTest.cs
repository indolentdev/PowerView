...
namespace PowerView.Model.Test.SeriesGenerators
{
  ...
  public class PeriodSeriesGeneratorTest
  {
    ...
    [Test]
    public void GenerateSameDeviceIdOld()
    {
      // Arrange
      var unit = Unit.WattHour;
      var dt = new DateTime(2015, 02, 13, 00, 01, 00, DateTimeKind.Local).ToUniversalTime();
      ...
      var periodValues = new[] {
        ...
        Normalize(new TimeRegisterValue("1", dt.AddHours(3), 30, unit)),
        Normalize(new TimeRegisterValue("1", dt.AddHours(4), 40, unit)),
        Normalize(new TimeRegisterValue("1", dt.AddHours(5), 50, unit)),
        Normalize(new TimeRegisterValue("1", dt.AddHours(6), 60, unit)),
        Normalize(new TimeRegisterValue("1", dt.AddHours(7), 100, unit)),
        Normalize(new TimeRegisterValue("1", dt.AddHours(8), 140, unit)),
        Normalize(new TimeRegisterValue("1", dt.AddHours(9), 150, unit)),
      };
      Assert.That(generatedValues, Is.EqualTo(periodValues));
    }

    [Test]
    public void GenerateCrossingDeviceIdOld()
    {
      // Arrange
      var unit = Unit.WattHour;
      var dt = new DateTime(2015, 02, 13, 00, 01, 00, DateTimeKind.Local).ToUniversalTime();
    ...
    [Test]
    public void GenerateCrossingDeviceIdAndBackOld()
    {
      // Arrange
      var unit = Unit.WattHour;
      var dt = new DateTime(2015, 02, 13, 00, 01, 00, DateTimeKind.Local).ToUniversalTime();
    ...
    [Test]
    public void GenerateCrossingDeviceIdWithOneValueAndBackOld()
    {
      // Arrange
      var unit = Unit.WattHour;
      var dt = new DateTime(2015, 02, 13, 00, 01, 00, DateTimeKind.Local).ToUniversalTime();
