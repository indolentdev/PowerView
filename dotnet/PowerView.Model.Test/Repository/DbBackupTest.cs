...
namespace PowerView.Model.Test.Repository
{
  ...
  public class DbBackupTest
  {
    ...
    public void ConstructorThrows()
    {
      ...
      // Act & Assert
      Assert.That(() => new DbBackup(null, DbName, minTimeSpan, maxBackupCount), Throws.TypeOf<ArgumentNullException>());
      Assert.That(() => new DbBackup(new NullLogger<DbBackup>(), null, minTimeSpan, maxBackupCount), Throws.TypeOf<ArgumentNullException>());
      Assert.That(() => new DbBackup(new NullLogger<DbBackup>(), string.Empty, minTimeSpan, maxBackupCount), Throws.TypeOf<ArgumentNullException>());
      Assert.That(() => new DbBackup(new NullLogger<DbBackup>(), DbName, minTimeSpan-TimeSpan.FromDays(1), maxBackupCount), Throws.TypeOf<ArgumentOutOfRangeException>());
      Assert.That(() => new DbBackup(new NullLogger<DbBackup>(), DbName, minTimeSpan, 0), Throws.TypeOf<ArgumentOutOfRangeException>());
    }
    ...
    private static DbBackup CreateTarget()
    {
      ...
      return new DbBackup(new NullLogger<DbBackup>(), DbName, TimeSpan.FromDays(14), 4);
    }
