...
namespace PowerView.Model.Repository
{
  ...
  {
    private readonly ILogger<DbUpgrade> logger;
    ...
    private IEnumerable<DbUpgradeResource> GetUpgradeManifestResources()
    {
      ...
      foreach (var resourceName in resourceNames.Where(rn => rn.StartsWith(startResourceName, StringComparison.InvariantCulture)))
      {
        ...
        {
          logger.LogInformation($"Unable to parse db version element from resource upgrade name. Skipping resource. ResourceName:{resourceName}, VersionElement:{versionElement}");
          continue;
        }
