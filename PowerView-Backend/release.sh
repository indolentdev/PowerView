...
perl -p -e 's/\n/\r\n/' < PowerView/bin/Release/net8.0/publish/appsettings.json > PowerView/bin/Release/net8.0/publish/appsettings.json_win
retVal=$?
if [ $retVal -ne 0 ]; then
  echo appsettings.json fiddling 1/3 returned $retVal
  exit 1
fi
rm PowerView/bin/Release/net8.0/publish/appsettings.json
retVal=$?
if [ $retVal -ne 0 ]; then
  echo appsettings.json fiddling 2/3 returned $retVal
  exit 1
fi
mv PowerView/bin/Release/net8.0/publish/appsettings.json_win PowerView/bin/Release/net8.0/publish/appsettings.json
retVal=$?
if [ $retVal -ne 0 ]; then
...
perl -p -e 's/\n/\r\n/' < PowerView/bin/Release/net8.0/publish/NLog.config > PowerView/bin/Release/net8.0/publish/NLog.config_win
retVal=$?
if [ $retVal -ne 0 ]; then
  echo NLog.config fiddling 1/3 returned $retVal
  exit 1
fi
rm PowerView/bin/Release/net8.0/publish/NLog.config
retVal=$?
if [ $retVal -ne 0 ]; then
  echo NLog.config fiddling 2/3 returned $retVal
  exit 1
fi
mv PowerView/bin/Release/net8.0/publish/NLog.config_win PowerView/bin/Release/net8.0/publish/NLog.config
retVal=$?
if [ $retVal -ne 0 ]; then
