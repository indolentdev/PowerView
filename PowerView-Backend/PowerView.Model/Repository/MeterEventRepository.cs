    ...
    internal class MeterEventRepository : RepositoryBase, IMeterEventRepository
    {
      ...
      ORDER BY [DetectTimestamp]";
        ...
        public WithCount<ICollection<MeterEvent>> GetMeterEvents(int skip = 0, int take = 50)
        {
            ...
            if (take < 1 || take > 1000) throw new ArgumentOutOfRangeException(nameof(take), $"Must be between 1 and 1000. Was:{take}");
      ...
      LIMIT @Take OFFSET @Skip";
            using (var transaction = DbContext.BeginTransaction())
            {
                try
                {
                    resultSet = DbContext.Connection.Query<RowLocal>(sql, new { Take = take, Skip = skip }, transaction, buffered: true);
        ...
        private static Db.MeterEvent ToDbEntity(MeterEvent meterEvent)
        {
            ...
            return new Db.MeterEvent
            {
                Label = meterEvent.Label,
                MeterEventType = meterEvent.Amplification.GetMeterEventType(),
                DetectTimestamp = meterEvent.DetectTimestamp,
                Flag = meterEvent.Flag,
